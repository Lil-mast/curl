...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const detailsHref = lastOpportunityId
    ? `/opportunities/${lastOpportunityId}`
    : "/opportunities/evening-english";
    ...
    <div className="dashboard-root min-h-dvh bg-paper lg:grid lg:grid-cols-[16.5rem_1fr]">
      <aside className="hidden flex-col bg-pine text-paper lg:flex">
        ...
        <div className="px-5 py-5">
          <p className="mb-2 text-xs font-bold uppercase tracking-wide text-sage">
            {t("language", lang)}
          </p>
          <div className="flex gap-2">
            <button
              type="button"
              className={`btn ${lang === "en" ? "btn-primary" : "btn-ghost text-paper"}`}
              onClick={() => setLang("en")}
            >
              EN
            </button>
            <button
              type="button"
              className={`btn ${lang === "so" ? "btn-primary" : "btn-ghost text-paper"}`}
              onClick={() => setLang("so")}
            >
              SO
            </button>
      ...
      <div className="flex min-h-dvh flex-col">
        ...
        <main className="flex-1 px-4 py-5 pb-32 lg:px-8 lg:py-8 lg:pb-8">
          {children}
        </main>
        ...
        <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-5 border-t border-line bg-card px-1 py-2 lg:hidden">
          {[
            ...
            <Link
              key={item.href}
              href={item.href}
              className="flex flex-col items-center gap-1 px-1 text-[0.65rem] font-semibold"
            >
              <item.icon />
